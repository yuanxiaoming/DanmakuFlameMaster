...
public abstract class GLHandlerSurfaceView extends SurfaceView
        implements SurfaceHolder.Callback2, GLShareable {
    ...
    private final static boolean LOG_RENDERER_DRAW_FRAME = GLConstants.LOG_RENDERER_DRAW_FRAME;
    private final static boolean LOG_EGL = GLConstants.LOG_EGL;
    ...
    public void exit() {
        ...
        if (DEBUG) {
            DanmakuLoggers
                    .i(TAG, "exit state=" + exitSucceed + "\t time=" + (System.nanoTime() - start));
        }
    }
    ...
    static class GLHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            try {
                while (true) {
                        ...
                        while (true) {
                            ...
                            if (readyToDraw()) {
                                ...
                                if (finishDrawingRunnable != null) {
                                    DanmakuLoggers
                                            .w(TAG, "Warning, !readyToDraw() but waiting for " +
                            ...
                            if (LOG_THREADS) {
                                DanmakuLoggers.i(TAG,
                                        "waiting tid=" + Thread.currentThread().getId()
                    ...
                    if (createEglSurface) {
                        if (LOG_SURFACE) {
                            DanmakuLoggers.w(TAG, "egl createSurface");
                        }
                    ...
                    if (createEglContext) {
                        if (LOG_RENDERER) {
                            DanmakuLoggers.w(TAG, "onSurfaceCreated");
                        }
                    ...
                    if (sizeChanged) {
                        if (LOG_RENDERER) {
                            DanmakuLoggers.w(TAG, "onSurfaceChanged(" + w + ", " + h + ")");
                        }
                    ...
                    if (LOG_RENDERER_DRAW_FRAME) {
                        DanmakuLoggers
                                .w(TAG, "onDrawFrame tid=" + Thread.currentThread().getId());
                    }
                    ...
                    switch (swapError) {
                        ...
                        case EGL11.EGL_CONTEXT_LOST:
                            if (LOG_SURFACE) {
                                DanmakuLoggers.i(TAG,
                                        "egl context lost tid=" + Thread.currentThread().getId());
                            }
                            lostEglContext = true;
                            break;
                        default:
        ...
        public void surfaceCreated() {
            ...
            synchronized (glHandlerSurfaceView.mMonitor) {
                if (LOG_THREADS) {
                    DanmakuLoggers
                            .i(TAG, "surfaceCreated tid=" + getLooper().getThread().getId());
                }
                mHasSurface = true;
        ...
        public void surfaceDestroyed() {
            ...
            synchronized (glHandlerSurfaceView.mMonitor) {
                if (LOG_THREADS) {
                    DanmakuLoggers.i(TAG,
                            "surfaceDestroyed tid=" + getLooper().getThread().getId());
                }
                mHasSurface = false;
                removeMessages(MSG_RUN);
                if (isLooperThread()) {
        ...
        public void onPause() {
            ...
            synchronized (glHandlerSurfaceView.mMonitor) {
                if (LOG_PAUSE_RESUME) {
                    DanmakuLoggers.i(TAG, "onPause tid=" + getLooper().getThread().getId());
                }
                mRequestPaused = true;
                removeMessages(MSG_RUN);
                if (isLooperThread()) {
        ...
        public void onResume() {
            ...
            synchronized (glHandlerSurfaceView.mMonitor) {
                if (LOG_PAUSE_RESUME) {
                    DanmakuLoggers.i(TAG, "onResume tid=" + getLooper().getThread().getId());
                }
            ...
            synchronized (glHandlerSurfaceView.mMonitor) {
                ...
                if (isLooperThread()) {
                    ...
                    while (!mExited && !mPaused && !mRenderComplete
                            && ableToDraw()) {
                        if (LOG_SURFACE) {
                            DanmakuLoggers.i("Main thread",
                                    "onWindowResize waiting for render complete from tid=" +
                                            getLooper().getThread().getId());
                        }
                        try {
                            glHandlerSurfaceView.mMonitor.wait(NORMAL_WAIT_TIME);
                        } catch (InterruptedException ignore) {
    ...
    private static class EglHelper {
        ...
        private final WeakReference<GLHandlerSurfaceView> mGLSurfaceViewWeakRef;
        ...
        public static void throwEglException(String function, int error) {
            ...
            if (LOG_THREADS) {
                DanmakuLoggers.e("EglHelper",
                        "throwEglException tid=" + Thread.currentThread().getId() + " "
                                + message);
            }
        ...
        public boolean createSurface() {
            if (LOG_EGL) {
                DanmakuLoggers
                        .w("EglHelper", "createSurface()  tid=" + Thread.currentThread().getId());
            }
            if (mEgl == null) {
            ...
            if (mEglSurface == null || mEglSurface == EGL10.EGL_NO_SURFACE) {
                ...
                if (error == EGL10.EGL_BAD_NATIVE_WINDOW) {
                    DanmakuLoggers
                            .e("EglHelper", "createWindowSurface returned EGL_BAD_NATIVE_WINDOW.");
                }
                return false;
            }
        ...
        public void destroySurface() {
            if (LOG_EGL) {
                DanmakuLoggers
                        .w("EglHelper", "destroySurface()  tid=" + Thread.currentThread().getId());
            }
            destroySurfaceImp();
        }
    ...
    private class DefaultContextFactory implements EGLContextFactory {
        ...
        public void destroyContext(EGL10 egl, EGLDisplay display,
            ...
            if (!egl.eglDestroyContext(display, context)) {
                DanmakuLoggers
                        .e("DefaultContextFactory", "display:" + display + " context: " + context);
                if (LOG_THREADS) {
                    DanmakuLoggers
                            .i("DefaultContextFactory", "tid=" + Thread.currentThread().getId());
                }
